...
$$
\sum_{p_0=2,3} \nu_{p_0}\left(\prod \tuple^{(3)}\right) \log p_0 \ll \sum_{p_0=2,3} \sum_{m \leq K} (\log\log N) \frac{t}{m \log N}$$
and from \eqref{nupsum} we obtain \eqref{23-est}.  

Applying similar estimates to \eqref{zp-def} gives
\begin{align*}
  Z_{p_1} \ll
\frac{1}{N} \sum_{K < m \leq K(1+\sigma): p_1|m} (\log\log N) \frac{\sigma t}{K \log N} \\
&\ll \frac{\log\log N}{AK\log N} (\frac{K\sigma}{p_1}+1) \\
&\ll \frac{\log\log N}{A p_1 \log^2 N} + \frac{\log\log N}{A K \log N}
\end{align*}
