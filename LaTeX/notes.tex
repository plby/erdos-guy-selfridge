...
In \cite[Theorem 2]{buthe-2} the bound
  $$ \psi(x) = x + O_{\leq}(0.94 \sqrt{x})$$
was demonstrated for $11 \leq x \leq 10^{19}$.  For $10^{19} \leq x \leq 10^{21} \approx e^{48.35}$, the slightly weaker bounds
  $$ \psi(x) = x + O_{\leq}(\frac{\sqrt{x}}{8\pi} \log x(\log x - 3))$$
were obtained in \cite[(7.3)]{buthe}.  On the other hand, in \cite[Theorem 1, Table 1]{buthe} the bound
$$ \psi(x) = x + O_{\leq}(1.11742 \times 10^{-8} x)$$
was shown for $x \geq e^{45} \approx 10^{19.54}$.  We may  combine these bounds together to obtain
\begin{equation}\label{psi-combined}
  \psi(x) = x + O_{\leq}(E(x))
\end{equation}
for all $x \geq 11$, where the error function $E(x)$ is defined as
$$ E(x) \coloneqq 0.94 \sqrt{x} + \frac{\sqrt{x}}{8\pi} \log x(\log x - 3) 1_{x \geq 10^{19}} + 1.11742 \times 10^{-8} x 1_{x \geq e^{45}}.$$
Note that this function is monotone increasing.
...
\begin{enumerate}
\item[(0)] Select natural numbers $A,K$ such that $K^2 (1+\frac{3N/t}{A}) < t$.  There is some freedom to select parameters here, but roughly speaking one would like to have $1 \lll A \lll K \lll \sqrt{t}$.
...
\item \emph{Borderline small primes} $K < p \leq K(1+\frac{3N/t}{A})$.
\item \emph{Medium primes} $K(1+\frac{3N/t}{A}) < p \leq t/K$.
...
\begin{equation}\label{nup-21}
   \nu_p( N!/\tuple^{(2)} ) = \nu_p( N!/\tuple^{(1)})
\end{equation}
for medium primes $K(1+\frac{3N/t}{A}) < p \leq t/K$.  For small and borderline small primes $3 < p \leq K(1+\frac{3}{A})$ one has
...
$$ \nu_p(N!/\tuple^{(2)}) = 0$$
for large primes $p>t/K$, while for tiny primes $p=2,3$ we have
$$ \nu_p(N!/\tuple^{(2)}) = \nu_p(N!)$$


We now pass to $\tuple^{(3)}$ by performing Step 3 of the algorithm.  In other words, we add $\nu_p(N!)$ copies of $p \lceil t/p\rceil$ for each prime large $p > t/K$.  The $t$-excess is now given by
$$  \excess_t(\tuple^{(3)}) = \excess_t(\tuple^{(2)}) + \sum_{p > t/K} \nu_p(N!) \log \frac{\lceil t/p \rceil}{t/p}
$$
and hence by \eqref{excess-2-bound}
\begin{equation}\label{excess-eq-alt}
\excess_t(\tuple^{(3)}) = \frac{3N^2}{2tA} + 4 + \sum_{p > t/K} \nu_p(N!) \log \frac{\lceil t/p \rceil}{t/p}.
\end{equation}
By construction one has balance \eqref{3-balance} at primes large $p>t/K$, 
...
Now we work on the left-hand side of \eqref{new-balance-5}.  By repeating the proof of \Cref{upper-bound}, we see that
...
\end{equation}
We begin by considering medium primes $K(1+\frac{3N/t}{A}) < p \leq t/K$.  In this range, we have from \eqref{nup-21}, \eqref{nup}, \eqref{3-medium} that
...
Now we deal with the (technical) contribution of the borderline small $K < p \leq K(1+\frac{3N/t}{A})$.  We still have \eqref{3-medium}, but the second term in the right-hand side of \eqref{nup-2} has one non-trivial term, namely when $m=p$.  By the Brun--Titchmarsh theorem, the contribution of this term is $O(\frac{N}{K \log N})$.  Hence in this range we have the crude bound
...
Now we consider the small primes $3 < p \leq K$.  From \eqref{mop}, \eqref{nup-2}, \eqref{nup} and the Brun--Titchmarsh inequality we have the crude  upper bound
...
\end{split}
\end{equation}
We can thus control the net contribution of the surpluses at small primes by
\begin{align*}
...
\end{align*}
by Mertens' theorem.  Hence this contribution is acceptable.  It remains to control the deficits at small primes:
...
\begin{align*}
\sum_{6a-1 \leq n \leq 6a+4} \frac{3}{n} 1_{(n,6)=1} - \frac{1}{n } = (\frac{1}{6a-1} - \frac{1}{6a}) + (\frac{1}{6a-1} - \frac{1}{6a+2}) + (\frac{1}{6a+1} - \frac{1}{6a+3}) +
+ (\frac{
  1}{6a+1} - \frac{1}{6a+4}) > 0.
\end{align*}
Similarly, on a block $6a-4 \leq n \leq 6a+1$, the sum is negative:
\begin{align*}
  \sum_{6a-4 \leq n \leq 6a+1} \frac{3}{n} 1_{(n,6)=1} - \frac{1}{n } = (\frac{1}{6a+1} - \frac{1}{6a}) + (\frac{1}{6a+1} - \frac{1}{6a-2}) + (\frac{1}{6a-1} - \frac{1}{6a-3}) +
  + (\frac{1}{6a-1} - \frac{1}{6a-4}) < 0.
\end{align*}
...
For the contribution of medium primes $K(1+\frac{3N/t}{A}) < p \leq t/K$ this follows readily from \eqref{anp}; the contribution of borderline small primes $K < p \leq K(1+\frac{3N/t}{A})$ follows readily from \eqref{tech}; and the contribution of small primes $3 < p \leq K$ follows readily from \eqref{crude-upper}, \eqref{nup3-lower}, and Mertens' theorem.  Also, from \eqref{tinycon}, \eqref{legendre} we see for a tiny prime $p=2,3$ that
\begin{align*}
...
\begin{align*}
  ...
  L &\coloneqq 342.
\end{align*}
Clearly $\delta = \log \frac{3}{e} = 0.09861\dots$.   From \Cref{lemcount-0}, we have
\begin{equation}\label{kappa-k}
  \kappa_K  = \kappa_L \leq \log \frac{9}{8} = 0.11778\dots
\end{equation}
Thus the right-hand side of \eqref{new-balance-5} is at least
...
At primes $p > N/3K$, the von Mangoldt function $\Lambda(p)$ is lower bounded by $\log N/3K$, hence
$$\excess_t(\tuple^{(3)}) \leq \frac{9N}{2A} + 4 + \frac{1}{\log(N/3K)} \sum_{N/3K < n \leq N} \Lambda(n) f(n/N)$$
...
Applying \Cref{integ-lemma} and \eqref{psi-combined}, we thus have
$$\excess_t(\tuple^{(3)}) \leq \frac{9N}{2A} + 4 + \frac{N}{\log(N/3K)} \int_{1/3K}^1 f(x)\ dx
