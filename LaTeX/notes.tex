...
\frac{\nu_2(\lceil x \rceil^{\langle 2,3\rangle}_L) - 2 \gamma \nu_3(\lceil x \rceil^{\langle 2,3\rangle}_L)}{1-\gamma} \leq \frac{1}{\log \sqrt{12}} \left( \log x  + \kappa^{(2)}_{L,\gamma} \right)
\end{equation}
and
\begin{equation}\label{12-3}
\frac{2\nu_3(\lceil x \rceil^{\langle 2,3\rangle}_L) - \gamma \nu_2(\lceil x \rceil^{\langle 2,3\rangle}_L)}{1-\gamma} \leq \frac{1}{\log \sqrt{12}} \left( \log x + \kappa^{(3)}_{L,\gamma} \right)
\end{equation}
where
...
\begin{equation}\label{kappastar-3-def}
  \kappa^{(3)}_{L,\gamma} \coloneqq \left(\frac{2\log \sqrt{12}}{(1-\gamma)\log 3} - 1\right) \log(12L) + \frac{2\kappa_L\log \sqrt{12}}{(1-\gamma)\log 3}.
\end{equation}
\end{itemize}
\end{lemma}

We remark that when $x$ is a power of $12$, the left-hand sides of \eqref{12-2}, \eqref{12-3} are both equal to $\frac{1}{\log \sqrt{12}} \log x$; thus the estimates \eqref{12-2}, \eqref{12-3} are quite efficient asymptotically.
...
\alpha_3 &\coloneqq \frac{1}{N\log \sqrt{12}} \left(\log \frac{t}{K} + \kappa_{**}\right) \sum_{3 < p_1 \leq t/K} \left|\nu_{p_1}\left( \frac{N!}{\prod \tuple^{(0)}} \right)\right| \label{alpha3-def}  \\
...
Note that the quantities $\delta_2, \delta$ essentially appeared previously \Cref{upper-crit}.  Informally, $\delta$ and the condition \eqref{delta-cond} represent the ``budget'' of what one can ``spend'' on various imperfections of the approximate factorization $\tuple^{(0)}$ until we become unable to ``afford'' to ``buy'' a $t$-admissible subfactorization of cardinality at least $N$.  The quantity $\delta_2$ represents the ``fixed expense'' coming from large primes that is unavoidable; but the other expenditures $\delta_i, i \neq 2$ can be made small by suitable exploitation of the pool of tiny primes present in the factorization of $N!$ (which multiply out to roughly $\sqrt{12}^N$).  The secondary constraint \eqref{alpha-cond} can roughly speaking be viewed as a requirement that one does not completely exhaust this pool.
...
The influence of the parameters $A,K,L$ on the other parameters $\delta_i, \alpha_i$ can be roughly summarized as follows (assuming that $L$ is significantly smaller than $N$):
\begin{itemize}
...
\item $\delta_7$ will be comparable to $\kappa_L/\log N$.
\item $\delta_4, \delta_6, \delta_8, \alpha_4, \alpha_6, \alpha_7$ will be lower order terms.
\item $\gamma_2, \gamma_3 \asymp \log L / \log N$; if this quantity is small enough, then $\kappa_{**} \asymp 1$.
\end{itemize}
...
and
$$ \alpha_3 \leq \frac{4A+3}{3N\log \sqrt{12}} \left(\log \frac{t}{K} + \kappa_{**}\right) \left(\pi(t/K) + \frac{\log N}{\log 5} \pi(\sqrt{N})\right).$$
\end{corollary}
...
One can readily check the required conditions \eqref{conditions}, \eqref{t-lower} for $N \geq N_0$, so it remains to verify the hypotheses \eqref{delta-cond}, \eqref{alpha-cond} of \Cref{repair} in this range.  Some of the quantities in these hypotheses involve sums over large ranges, such as $(t/K,N]$; but one can use \Cref{osc-lemma} to obtain adequate upper or lower bounds on such quantities, leaving one with sums over short ranges such as $p \leq K$ or $p \leq K(1+\sigma)$.  As such, all of the bounds needed cna be quickly computed even for very large $N$ with simple computer code\footnote{\url{https://github.com/teorth/erdos-guy-selfridge/blob/main/src/python/interval_computations.py}}.
...
$$ \delta_1 \leq \frac{9}{2A} + \frac{4}{N_0} \leq 0.023810 \delta$$
for all $N \geq N_0$.
...
\begin{align*}
  ...
  &\leq \frac{1}{\log(N_0/3K)} 0.919785 + \frac{1}{\log(N_0/3K)} 1159.795 \frac{E(N_0)}{N_0} \\
  &\leq 0.504735 \delta
\end{align*}
for all $N \geq N_0$.  For $N \geq 10^{70}$ we may replace $N_0$ by $10^{70}$ and obtain the significantly better bound
...
\begin{align*}
   \alpha_6 &\leq \frac{1}{\log \sqrt{12}} \left( \frac{\log(N_0/3)}{N_0} + \frac{\kappa_{**}}{N_0} \right) \\
   &\leq 3 \times 10^{-10}
\end{align*}
...
$$ B_{p_1} \leq \sum_{m \leq K} \nu_{p_1}(m) \sum_{k: a_{k,m} < b_{k,m}} k \frac{1}{\log(N a_{k,m})} \left( a_{k,m}-b_{k,m} + 2\frac{E(N_0 b_{k,m})}{N_0 \log(N a_{k,m})} \right)$$
and lower bound it by
$$ B_{p_1} \geq \sum_{m \leq K} \nu_{p_1}(m) \sum_{k: a_{k,m} < b_{k,m}} k \frac{1}{\log(N b_{k,m})} \left( \left(1-\frac{2}{\sqrt{a_{k,m} N}}\right) (a_{k,m}-b_{k,m}) - 2\frac{E(N_0 b_{k,m})}{N_0 b_{k,m} \log(N b_{k,m})} \right).$$
...
From the monotonicity of the upper bound, one can use \eqref{alpha2-def} to calculate that that
$$ \alpha_2 \leq 0.270233$$
for all $N \geq N_0$.  For \eqref{delta7-def}, subtraction is involved, and one must proceed with more caution.  For $N=N_0$, one has
$$ \delta_7 \leq 0.11363 \delta.$$
For $N \geq 10^{70}$, we simply discard the negative terms here and obtain the bound
$$ \delta_7 \leq \frac{\kappa_{4.5}}{\log(10^{70}/3)} \log \sqrt{12} \leq 0.02265 \delta$$
...
\begin{align*}
\delta_5 &\leq 0.062824 \delta \\
\alpha_5 &\leq 0.315286
\end{align*}
which, when combined with the previous bounds, gives
$$ \sum_{i=1}^8 \delta_i  \leq 0.9742 \delta$$
and 
$$ \sum_{i=1}^7 \alpha_i \leq 0.9467$$
...
and
$$ \alpha_5 \leq \frac{1}{\log \sqrt{12}} \sum_{3 < p_1 \leq K} (\log K^2+\kappa_{**}) \frac{\log p_1}{\log(t/K^2)} A_{p_1} + (\log p_1 + \kappa_{**}) B_{p_1}.$$
The bounds available for the right-hand side are now monotone in $N$, and one can calculate that
\begin{align*}
  \delta_5 &\leq 0.077304 \delta \\
  \alpha_5 &\leq 0.184984
  \end{align*}
...
$\delta_5, \delta_7, \alpha_2, \alpha_5$, thus potentially verifying the conditions \eqref{delta-cond}, \eqref{alpha-cond} simultaneously for all $N$ in such an interval.  As it turns out, there is enough room to spare in theese estimates, particularly for large $N$, that this strategy works using only a small number of intervals; specifically, by considering $N$ in the intervals
...
\begin{align*}
  ...
  &= \frac{2\log x}{\log 12} + \left( \frac{1}{(1-\gamma)\log 2} - \frac{1}{\log \sqrt{12}}\right) \left(\log x - a \log 12\right)
...
\begin{align*}
  ...
  &= \frac{2\log x}{\log 12} + \left( \frac{2}{(1-\gamma)\log 3} - \frac{1}{\log \sqrt{12}}\right) \left(\log x - a \log 12\right)
