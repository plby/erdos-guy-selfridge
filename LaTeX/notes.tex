...
and
 \begin{align*}
  &\frac{1}{N} \sup_{p_0=2,3} (p_0-1) \nu_{p_0}\left(\prod \tuple^{(4)}\right) + \frac{2}{\log 3} (\log(K^2)+\kappa_K) Y_1 + \frac{2}{\log 3} (\log(t/K)+\kappa_K) (Y_2+\frac{1}{N}) \\
&\quad
  \leq 1 - \alpha.
 \end{align*}
Then $t(N) \geq t$.
...
For each of the $M$ factors $c_i$, we introduce the $3$-smooth number $\lceil t/c_i\rceil^{\langle 2,3\rangle} = 2^{n_i} 3^{m_i}$, which by \eqref{kappa-def} lies in the interval $[t/c_i,e^{\kappa_K} t/c_i]$; similarly, for the exceptional factor $c_*$ we introduce a $3$-smooth number $\lceil t/c_* \rceil^{\langle 2,3 \rangle} = 2^{n_*} 3^{m_*}$ in the interval $[t/c_*,e^{\kappa_K} t/c_*]$.  Finally, for the $Y_2$ borderline or medium primes $K < p \leq t/K$ discussed above, we introduce the $3$-smooth number $\lceil t/p\rceil^{\langle 2,3\rangle} = 2^{n_p} 3^{m_p}$, which lies in the interval $[t/p,e^{\kappa_K} t/p]$.  We adjoin the $3$-smooth numbers $\lceil t/c_i\rceil^{\langle 2,3\rangle} c_i = 2^{n_i} 3^{m_i} c_i$ for $i=1,\dots,M$ as well $\lceil t/c_*\rceil^{\langle 2,3\rangle} c_* = 2^{n_*} 3^{m_*} c_*$ and $\lceil t/p \rceil^{\langle 2,3\rangle} p = 2^{n'_p} 3^{m'_p} c_p$ to the $t$-admissible multiset $\tuple^{(4)}$ to create a new $t$-admissible multiset $\tuple^{(5)}$.  

For each $i=1,\dots,M$, the quantities $\log \lceil t/c_i\rceil^{\langle 2,3\rangle} = n_i \log 2 + m_i \log 3$ are bounded by $\log K^2 + \kappa_K$, and hence
$$ \max( n_i, 2m_i ) \leq \frac{2}{\log 3} (\log K^2 + \kappa_K).$$
Similarly we have
$$ \max( n_*, 2m_* ) \leq \frac{2}{\log 3} (\log (t/K) + \kappa_K)$$
and
$$ \max( n'_p, 2m'_p ) \leq \frac{2}{\log 3} (\log (t/K) + \kappa_K)$$
for the medium primes $m$.
Hence
\begin{align*} 
  & \sup_{p_0=2,3} (p-1) \nu_{p_0}\left(\prod \tuple^{(5)}\right) \\
  &\quad \leq \sup_{p_0=2,3} (p-1) \nu_{p_0}\left(\prod \tuple^{(4)}\right) \log p_0 \\
  &\quad + \frac{2}{\log 3} NY_1 (\log K^2 + \kappa_K) + \frac{2}{\log 3} (NY_2+1) (\log(t/K) + \kappa_K).
\end{align*}
...
&\quad + \frac{2}{\log 3} (\log K^2 +\kappa_K) Y_1^+ + \frac{2}{\log 3} (\log K + \kappa_5) Y_1^- + \frac{2}{\log 3} (\log(t/K) +\kappa_K) (Y_2^\pm+\frac{1}{N})  
\\
  &\quad  \leq 1-\alpha
  \end{split}
\end{equation}
...
\sum_{p_0=2,3} (p_0-1) \nu_{p_0}\left(\prod \tuple^{(3)}\right) + \frac{2}{\log 3} \sum_{p>3} \left|\nu_p\left(\frac{N!}{\prod \tuple^{(3)}}\right)\right|_{0, \log p+\kappa_{p}}$$
and hence
\begin{align*}
  &\frac{1}{N} \sup_{p_0=2,3} (p_0-1) \nu_{p_0}\left(\prod \tuple^{(4)}\right) + \frac{2}{\log 3} (\log(K^2)+\kappa_K) Y_1 + \frac{2}{\log 3} (\log(t/K)+\kappa_K) (Y_2+\frac{1}{N}) \\
  &\quad \leq 
  \frac{1}{N} \sup_{p_0=2,3} (p_0-1) \nu_{p_0}\left(\prod \tuple^{(3)}\right)+ \frac{2}{\log 3}  (\log(K^2)+\kappa_K) Y^+_1 \\
  &\quad \quad + \frac{2}{\log 3} (\log K + \kappa_K) Y^-_1 + \frac{2}{\log 3} (\log(t/K)+\kappa_K) (Y^\pm_2+\frac{1}{N}).
\end{align*}
