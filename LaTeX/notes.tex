...
\begin{equation}\label{pj}
 ...
 \begin{equation}\label{hyp}
  \sum_p w_p \nu_p(N!) < N,
  \end{equation}
  then by multiplying \eqref{constraints} by $w_p$ and summing over $p \leq N$, we see that these constraints force $\sum_{j \in J_{t,N}} m_j < N$, and hence $t(N) < t$.  We call this the \emph{linear programming upper bound}.  In practice, this bound is quite sharp, and can be readily computed for $N$ up to $10^4$ without difficulty.
  
Linear programming can also be used to obtain a somewhat crude lower bound, as follows.  First, one uses linear programming to maximize the quantity $\sum_{j \in J_{t,N}} m_j$ for non-negative \emph{reals} $m_j$ subject to the constraints \eqref{conditions}.  If one has $\sum_{j \in J_{t,N}} \lfloor m_j \rfloor \geq N$, then $\prod_{j \in J_{t,N}} j^{\lfloor m_j \rfloor}$ represents a $t$-admissible subfactorization of $N!$ that witnesses $t(N) \geq t$.  We call this the \emph{linear programming floor lower bound}.  In practice one can improve on this bound slightly by gathering all remaining primes not exhausted by this subfactorization (thus each $p$ remains with multiplicity $\nu_p(N!) - \sum_{j \geq t} \lfloor m_j\rfloor \nu_p(j)$), and applying a greedy algorithm to this collection to fashion a small number of additional $t$-admissible factors to add to the subfactorization; let us call this the \emph{linear programming floor+residuals lower bound}.

For small values of $N$, such as $N \leq 10^3$, the linear programming method is extremely accurate in upper and lower bounding $t(N)$; for instance, these methods give the exact value of $t(N)$ for all $N \leq 600$, with the sole exception of $N=155$, where the linear programming bounds instead give $45 \leq t(155) \leq 46$.  In this case, the (slower) integer programming method can be deployed to verify that $t(155)=45$.
...
\begin{lemma}[Linear programming bound]\label{lp-upper}  Let $N$ be an natural number and $1 \leq t \leq N/2$.  Suppose for each prime $p \leq N$, one has a non-negative real number $w_p$ which is weakly non-decreasing in $p$ (thus $w_p \leq w_{p'}$ when $p \leq p'$), and such that \eqref{pj} holds for all $t \leq j \leq N$, and such that \eqref{hyp} also holds.  Then $t(N) < t$.
\end{lemma}
  
\begin{proof}
By the linear programming upper bound, it will suffice to show that the bound \eqref{pj} in fact holds for all $j \geq t$, not just for $t \leq j \leq N$.  Indeed, if this were not the case, consider the first $j \geq t$ where \eqref{pj} fails.  Take a prime $p$ dividing $j$ and replace it by a prime in the interval $[p/2,p)$ which exists by Bertrand's postulate (or remove $p$ entirely, if $p=2$); this creates a new $j'$ in $[j/2,j)$ which is still at least $t$.  By the weakly decreasing hypothesis on $w_p$, we have
  $$ \sum_p w_p \nu_p(j) \geq \sum_p w_p \nu_p(j')$$
...
We have found empirically that using linear programming to minimize the left-hand side of \eqref{hyp} subject to \eqref{pj} for $t \leq j \leq N$ tends to generate weights $w_p$ that are in fact weakly decreasing, so that a rigorous upper bound $t(N) < t$ can be established by this method without needing to expand the linear program to all $j \in J_{t,N}$.  We were unable to rigorously demonstrate this weak decrease for this specific linear program, but we can show that weak decrease must occur when one relaxes the constraint $t \leq j \leq N$ somewhat:

\begin{lemma}[Weakly increasing dual weights]\label{dual-weights}\ 
  \begin{itemize}
  \item[(i)] Let $w'_p \geq 0$, $p \leq N$ be weights that minimize the left-hand side of \eqref{hyp} subject to \eqref{pj} holding for \emph{all} $j \geq t$.  Then the $w'_p$ are weakly increasing in $p$.
  \item[(ii)] Let $w''_p \geq 0$, $p \leq N$ be weights that minimize the left-hand side of \eqref{hyp} subject to \eqref{pj} holding for those $j \geq t$ that divide $N!$.  Suppose further that $t \leq N/2$. Then the $w''_p$ are weakly increasing in $p$.
  \end{itemize}
  \end{lemma}
  
  \begin{proof}  For (i), suppose for contradiction that there are primes $p_1 < p_2 \leq N$ such that $w'_{p_1} > w'_{p_2}$.  Let $\tilde w'_p$ be the weights formed by setting $\tilde w'_p \coloneqq w'_p$ for all $p \neq p_1$ and $\tilde w'_{p_1} \coloneqq w'_{p_2}$.  Then $\sum_p \tilde w'_p \nu_p(N!) < \sum_p w'_p \nu_p(N!)$. On the other hand, for any $j \geq t$, we see that
    $$ \sum_p \tilde w'_p \nu_p(j) = \sum_p w'_p \nu_p(\tilde j)$$
    where $\tilde j$ is formed from $j$ by replacing all factors of $p_1$ in the prime factorization of $j$ with $p_2$.  Since $p_2 > p_1$, we have $\tilde j > j \geq t$, and so $\tilde w'_p$ also obeys the conditions \eqref{pj} for all $j \geq t$, contradicting the minimizing property of the $w'_p$.

  For (ii), we similarly suppose for contradiction that there are primes $p_1 < p_2 \leq N$ such that $w''_{p_1} > w''_{p_2}$.  Let $\eps>0$ be a sufficiently small quantity, and define the modification $\tilde w''_p$ to $w''_p$ by decreasing $w''_{p_1}$ by $\eps$ and leaving all other $w''_p$ unchanged.  This again decreases the left-hand side of \eqref{hyp}, so to get a contradiction as before, it suffices to show that
  $$ \sum_p \tilde w''_p \nu_p(j) \geq 1$$
...
This now gives a number of variants of the linear programming method to produce upper and lower bounds on $t(N)$ for medium values of $N$, say in the range $10^3 \leq N \leq 10^8$:
\begin{itemize}
\item[(i)] One can obtain a \emph{fast linear programming upper bound} by minimizing \eqref{hyp} subject to \eqref{pj} for $t \leq j \leq N$, checking that the resulting weights $w_p$ are weakly non-decreasing, and then applying \Cref{lp-upper}.  In principle, this method could fail by producing weights that are not weakly non-decreasing, but we have not encountered this scenario in practice.  This bound works well for $N$ as large as $8 \times 10^8$ (see \Cref{fig-longer}).
\item[(ii)] One can obtain various \emph{integer programming lower bounds} by applying integer programming (restricting to the range $t \leq j \leq N$ for speed) to see if a lower bound $t(N) \geq t$ can be obtained first for $t$ equal to the floor of the (fast) linear programming upper bound, then decrementing $t$ by one whenever the integer program fails to terminate in a reasonable length of time (e.g., a minute).  This works well in practice for $N$ up to about $10^5$; see \Cref{fig-surplus}, and when it succeeds, the subfactorizations produced by the method can be readily verified independently by other software tools.  However, the performance of this method is sensitive to implementation parameters, such as the length of time one spends attempting to solve the integer program for a given choice of parameters. 
\item[(iii)] We developed a more complicated\footnote{Details of this algorithm may be found at \url{https://github.com/teorth/erdos-guy-selfridge/tree/main/src/mojo}.} \emph{smooth factorization lower bound} which used the greedy approach from the previous section to deal with large primes and linear programming methods (using smooth factors) to deal with small primes, which works well in practice for {\bf describe range of $N$}, and outperforms the simpler linear programming floor lower bound, though worse than the (slower) impatient integer programming bound; see \Cref{fig-surplus}.
\end{itemize}

The fast linear programming upper bound, when combined with some \emph{ad hoc} integer programming bounds, can compute $t(N)$ exactly for all $N \leq 10^4$ with the exception of $N=155,765,1528,1618,1619,2574,2935,3265,5122,5680,9633$, but in these cases $t(N)$ can be computed exactly by the full integer program; see \cite{github} and \Cref{fig-long}.

One can also use this method to accurately bound subfactorizations of $N!$ for larger $N$, although the runtime becomes slow.  For instance, with $N = 3 \times 10^5$ and $t = N/3 = 10^5$, the fast linear program upper bound can be used to show that any $t$-admissible factorization has cardinality at most $N+455$, while the linear programming floor+residuals lower bound method produces a $t$-admissible factorization of exactly this cardinality.  This demonstrates \Cref{main}(ii), (iii) for this value of $N$.  The fast linear programming upper bound and \emph{ad hoc} integer program lower bounds can also establish \Cref{main}(iii) in the range $43632 \leq N \leq 8 \times 10^4$, but show that this conjecture fails for $N = 43631$; it also holds for $N=41006$, but fails for all smaller $N$ except for $N=1,2,3,4,5,6,9$; see \Cref{fig-surplus}.  When combined with the greedy algorithm computations from the previous section, this resolves \Cref{main}(ii), (iii) except in the asymptotic range $N > 10^{11}$, where it suffices to establish the lower bound $t(N) \geq N/3$.
...
\begin{figure}
  ...
  \caption{Exact values of $t(N)/N$ for $80 \leq N \leq 10^4$, obtained via fast linear and \emph{ad hoc} integer programming in most cases (and the full integer program in some exceptional cases).  The upper bound from \Cref{upper-crit} is surprisingly sharp, as is the refined asymptotic $1/e - c_0/\log N - c_1/\log^2 N$, though the cruder asymptotics $1/e$ or $1/e - c_0/\log N$ are significantly poorer approximations.}
  \label{fig-long}
  \end{figure}
  ...
  \begin{figure}
    ...
    \begin{figure}
      ...
      \caption{Bounds on $M(N)-N$ for $4 \times 10^5 \leq N \leq 4.5 \times 10^5$, where $M(N)$ is the largest cardinality of an $N/3$-admissible subfactorization of $N!$ (thus $t(N) \geq N/3$ if and only if $M(N)-N \geq 0$).  Fast linear programming provides an upper bound (red), which one can round down to the nearest integer.  \emph{Ad hoc} integer programming (green) provides lower bounds that almost always matches the upper bound (rounded down) at this range of $N$; in particular it verifies $t(N) \geq N/3$ for $43632 \leq N \leq 4.5 \times 10^5$.  The smooth factorization method (purple) gives slightly worse bounds than the(slower) ad hoc integer programming methods, but slightly better than the (faster) linear algebra floor method (blue).}
      \label{fig-surplus}
      \end{figure}
