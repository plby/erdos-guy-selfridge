...
In practice, $\lceil x \rceil^{\langle 2,3 \rangle}$ will only be slightly larger than $x$; we quantify this in \Cref{power-sec}.  Later in the paper we will also introduce a variant $\lceil x \rceil^{\langle 2,3 \rangle}_{4.5}$, which is also a $3$-smooth number slightly larger than $x$, but also close to a power of $12$.
...
$b(y^+) \coloneqq \lim_{t \to y^+} b(t)$ denotes the right limit of $b$ at $y$ (if it exists).  Equivalently, $\|b\|_{\mathrm{TV}^*(y,x]}$ is the total variation of $b$ if extended by zero outside of $(y,x]$.  The total variation is a useful tool for bounding weighted sums of functions whose partial sums are known to be under control; see \Cref{integ-lemma}.
...
\begin{lemma}[Efficient subfactorization of $3$-smooth numbers]\label{bound23}  Let $L \geq 1$.  Let $t > 3L$ and let $2^n 3^m$ be a $3$-smooth number with $n,m$ positive and obeying the condition
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{tinyfac.png}
    \caption{A plot of (the exponents of) the $3$-smooth numbers appearing in the proof of \Cref{bound23}.  Once one knows that $(n,m)$ is a positive linear combination of $(n_0+n_1,m_1)$ and $(n_2,m_0+m_2)$, one can efficiently find a subfactorization of $2^n 3^m$ into $2^{n_0+n_1} 3^{m_1}$ and $2^{n_2} 3^{m_0+m_2}$.}
    \label{fig-tinyfac}
    \end{figure}
...
Some efficient choices of parameters for this lemma are given in \Cref{approx-table}.  For instance, $\kappa_{4.5} \leq 0.28768\dots$ and $\kappa_{40.5} \leq 0.16989\dots$. The bounds given by this table are in fact exact: see \Cref{fig:nextsmooth}.
