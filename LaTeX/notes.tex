...
\end{equation}
for an explicit constant $c_1=0.755\dots$; see 
\end{itemize}
\end{theorem}
...
We can now prove the upper bound portion of \Cref{main}(iv):
  ...
  \begin{proposition}\label{upper-bound}  For large $N$, one has
    ...
    where
    \begin{align*}
      c'_1 &\coloneqq \frac{1}{e} \int_0^1 f_e(x) \log \frac{1}{x}\ dx \approx 0.370\\
      c''_1 &\coloneqq \sum_{k=1}^\infty \frac{1}{k}  \log\left( \frac{e}{k} \left\lceil \frac{k}{e} \right\rceil \right) \approx 1.680\\
      c_1 &\coloneqq c'_1 + c_0 c''_1 - ec_0^2/2 \approx 0.7552.
    \end{align*}
  \end{proposition}
