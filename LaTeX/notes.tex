...
\begin{itemize}
  ...
  \begin{equation}\label{pj}
   ...
   \begin{equation}\label{hyp}
    \sum_p w_p \nu_p(N!) < N,
    \end{equation}
    then by multiplying \eqref{constraints} by $w_p$ and summing over $p \leq N$, we see that these constraints force $\sum_{j \in J_{t,N}} m_j < N$, and hence $t(N) < t$.  We call this the \emph{linear programming upper bound}.
\end{itemize}

By the duality of linear programming, the linear programming floor lower bound and linear programming upper bound would match if the constraint $\sum_{j \in J_{t,N}} \lfloor m_j \rfloor \leq N$ in the lower bound were replaced with the linear constraint $\sum_{j \in J_{t,N}} m_j \leq N$.
...
\begin{figure}
  ...
  \caption{Exact values of $t(N)/N$ for $80 \leq N \leq 10^4$, obtained via fast linear and integer programming in most cases (and the full integer program in some exceptional cases).  The upper bound from \Cref{upper-crit} is surprisingly sharp, as is the refined asymptotic $1/e - c_0/\log N - c_1/\log^2 N$, though the cruder asymptotics $1/e$ or $1/e - c_0/\log N$ are significantly poorer approximations.}
  \label{fig-long}
  \end{figure}
  ...
  \begin{figure}
    ...
    \begin{figure}
      ...
      \caption{Bounds on $M(N)-N$ for $4 \times 10^5 \leq N \leq 4.5 \times 10^5$, where $M(N)$ is the largest cardinality of an $N/3$-admissible subfactorization of $N!$ (thus $t(N) \geq N/3$ if and only if $M(N)-N \geq 0$).  Fast linear programming provides an upper bound (red), which one can round down to the nearest integer.  Impatient integer programming (green) provides upper bounds that almost always matches the upper bound (rounded down) at this range of $N$; in particular it verifies $t(N) \geq N/3$ for $43632 \leq N \leq 4.5 \times 10^5$ {\bf not quite; there are four exceptions currently}.  The smooth factorization method (purple) gives slightly worse bounds than the(slower) impatient integer programming method, but slightly better than the (faster) linear algebra floor method (blue).}
      \label{fig-surplus}
      \end{figure}
