...
\end{equation}
One can also replace all occurrences of $E(x)$ here by $O(x \exp(-c \sqrt{\log x}))$ for some absolute constant $c>0$.
\end{lemma}
...
\begin{equation}\label{conditions}
    t \geq K^3, K \sqrt{N}; \quad K \geq 5.
\end{equation}
...
\delta_4 &\coloneqq \frac{1}{N} \kappa_{4.5} \sum_{K < p_1 \leq t/K} A_{p_1} \label{delta4-def}  \\
\delta_5 &\coloneqq \frac{1}{N} \kappa_{4.5} \sum_{3 < p_1 \leq K} |A_{p_1} - B_{p_1}|_{\frac{\log p_1}{\log(t/K^2)},1} \label{delta5-def}  \\
\delta_6 &\coloneqq \frac{1}{N} \kappa_{4.5} \label{delta6-def}  \\
\delta_7 &\coloneqq \frac{\log 12}{2\log t} \kappa_* \label{delta7-def}  \\
\delta_8 &\coloneqq \frac{(n_2+n_3)\log 2 + (m_2+m_3)\log 3}{N} \label{delta8-def}\\
...
\alpha_2 &\coloneqq \frac{1}{N} \left\| (B_2, B_3) \right\|_\gamma \label{alpha2-def}  \\
...
\alpha_4 &\coloneqq \frac{2}{N\log 12} \sum_{3 < p_1 \leq K} \left(\log \frac{t}{p_1} + \kappa_{**}\right) A_{p_1} \label{alpha4-def}  \\
\alpha_5 &\coloneqq \frac{2}{N\log 12} \sum_{3 < p_1 \leq K} \left|A_{p_1} - B_{p_1}\right|_{\frac{\log p_1}{\log(t/K^2)} (\log K^2 + \kappa_{**}), \log p_1 + \kappa^*_{4.5}} \label{alpha5-def}  \\
...
By \eqref{delta8-def}, we thus have
$$ \frac{1}{N} \sum_p \nu_p\left( \frac{N!}{\prod \tuple^{(3)}}\right) \log p \leq \delta_8$$
