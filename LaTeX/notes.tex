...
\begin{figure}
  ...
  \begin{figure}
    ...
    \caption{Upper and lower bounds $t(N)/N$ obtained by linear programming for $10^3 \leq N \leq 10^5$ that are multiples of $100$. The refined asymptotic $1/e - c_0/\log N - c_1/\log^2 N$ is now a slight underestimate, hinting at further terms in the asymptotic expansion.}
    \label{fig-longer}
    \end{figure}
...
\hline
\end{tabular}
\caption{For sample values of $N \in [10^6, 7 \times 10^8]$, the performance of the fast greedy algorithm (using heuristics to guess optimal choices of $t$), and more thorough run of the algorithm (that exhaustively searches over $t$ for which the algorithm works), compared against the lower bound $t(N)_-$ obtained from the linear programming method.}\label{greedy-table}
\end{table}

\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth]{verylarge.png}
  \caption{Lower bounds on $t(N)/N$ coming from the linear programming and fast greedy methods in the range $10^5 \leq N \leq 10^{12}$.  (The linear programming upper bound is not displayed, as it is visually indistinguishable from the lower bound.)}\label{fig-verylarge}
\end{figure}
...
$$ \sum_{0 \leq m \leq M}^* \frac{1}{2} \min( \frac{1}{2^m}, \frac{\alpha}{\ell}) \mu_+ + (1-\mu_+).$$
...
\begin{equation}\label{amp} 
  \sum_{0 \leq m \leq M}^* \frac{1}{2} \min(\frac{1}{2^m}, \frac{\alpha}{\ell})\mu_+ + \sum_{0 \leq m \leq M}^* \frac{1}{2} \sum_{p_- \leq p \leq p_+} \min(\frac{1}{2^mp}, \frac{\alpha}{\ell}) \mu_p.
\end{equation}
By \eqref{lint} it suffices to show that
$$
\sum_{p_+/2 \leq p \leq p_+} \sum_{m: 2^{m+C_0} p > \ell} \frac{m}{2^m} \frac{\mu_p}{p}
> \sum_{0 \leq m \leq M}^* \frac{1}{2} \sum_{p_- \leq p \leq p_+} \min(\frac{1}{2^mp}, \frac{\alpha}{\ell}) \mu_p.$$
...
&\leq \frac{1}{2^m \log((\ell/\alpha 2^m))} - \frac{1}{2^m \log p_+} + \frac{1}{2^m \log^2((\ell/\alpha 2^m))} + O_M\left( \frac{1}{\log^3 \ell}\right) \\
&=  \frac{1}{2^m \log \ell} - \frac{1}{2^m \log p_+} + \frac{m \log 2 - \log(e\alpha)}{2^m \log^2 \ell} + O_M\left( \frac{1}{\log^3 \ell}\right).
\end{align*}
Performing the $m$ summation, we conclude that the right-hand side of \eqref{test} is at most
$$ (\log p_-) \left(\frac{1}{\log \ell} - \frac{1}{\log p_+} + \frac{\log 2 - \log(e\alpha)}{\log^2 \ell} + O_M\left( \frac{1}{\log^3 \ell}\right) \right).$$
...
Applying \Cref{osc-lemma}, we can bound this by
$$
\left( 1 + O\left(\frac{1}{\log^{10} p_-}\right)\right)
...
Since $\alpha < 1/e$, we can ensure that $c_0 \log 2 < \log(e\alpha)$ by taking $c_0$ small enough.  The claim \eqref{test} then follows.
\end{proof}
