...
It will be convenient to introduce a variant of this quantity that is close to a power of $12$.  If $1 \leq L \leq x$ is an additional real parameter, we define
...
\begin{figure}
  ...
  \caption{The function $\log \lceil x \rceil^{\langle 2,3 \rangle}/x$, compared against $\kappa_x$. 
...
We use $a|b$ to denote the assertion that $a$ divides $b$, and $\pi(x) = \sum_{p \leq x} 1$ to denote the usual prime counting function. The effective and asymptotic estimates over primes that we will use are summarized in \Cref{primes-sec}.
...
$b(y^+) \coloneqq \lim_{t \to y^+} b(t)$ denotes the right limit of $b$ at $y$ (if it exists).  Equivalently, $\|b\|_{\mathrm{TV}^*(y,x]}$ is the total variation of $b$ if extended by zero outside of $(y,x]$. The indicator function $1_{(y,x]}$ clearly has an augumented total variation of $2$. 
...
\begin{equation}\label{tilde-e}
  E(x) \coloneqq 0.95 \sqrt{x} + 2.39 \times 10^{-8} x.
\end{equation}
In particular one has
\begin{equation}\label{pix}
  \pi(x) - \pi(y) = \int_y^x (1-\frac{2}{\sqrt{t}})\frac{dt}{\log t} + O_\leq(2 \frac{E(x)}{\log y}).
\end{equation}
...
\end{equation}
One can also replace all occurrences of $E(x)$ here by the classical error term $O(x \exp(-c \sqrt{\log x}))$ for some absolute constant $c>0$.
\end{lemma}

We remark that the accuracy in \eqref{bv-exact}, \eqref{pix} in particular is on par with what would be provided by the Riemann hypothesis, as long as $x$ is not too large (e.g., $x \leq 10^{16}$).  The other estimates are not quite as precise, but still adequate for our applications.  We also remark that the second term in \eqref{tilde-e} could be reduced by further numerical verification of the Riemann hypothesis, although this would not significantly simplify the analysis in this paper.
...
\begin{figure}
  ...
  \caption{$t(N)/N$ for $80 \leq N \leq 10^4$, obtained via linear programming in most cases (and integer programming in some exceptional cases).  The upper bound from \Cref{upper-crit} is surprisingly sharp; on the other hand, the asymptotic $1/e - c_0/\log N$ is a relatively poor approximation, suggesting the presence of a lower order term that empirically appears to be about $1/e - c_0/\log N - 0.7/\log^2 N$.}
  \label{fig-long}
  \end{figure}
...
\delta_3 &\coloneqq \frac{1}{N} \kappa_{4.5} \sum_{3 < p_1 \leq t/K} \left|\nu_{p_1}\left( \frac{N!}{\prod \tuple^{(0)}} \right)\right| \label{delta3-def}  \\
...
\delta_6 &\coloneqq \frac{1}{N} \kappa_{4.5} \label{delta6-def}  \\
\delta_7 &\coloneqq \frac{\log 12}{2\log t} \kappa_* \label{delta7-def}  \\
...
\alpha_5 &\coloneqq \frac{2}{\log 12} \sum_{3 < p_1 \leq K} \left|A_{p_1} - B_{p_1}\right|_{\frac{\log p_1}{\log(t/K^2)} (\log K^2 + \kappa_{**}), \log p_1 + \kappa^*_{4.5}} \label{alpha5-def}  \\
\alpha_6 &\coloneqq \frac{2}{N\log 12} (\log t + \kappa_{**}) \label{alpha6-def}  \\
\alpha_7 &\coloneqq \max\left(\frac{1}{N} \lceil \frac{\log N}{\log 3}\rceil, \frac{2}{N} \lceil \frac{\log N}{\log 3}\rceil\right)\label{alpha7-def}  \\
...
\end{align}
with the convention that the upper bound $p < \frac{t}{m-1}$ is vacuous when $m=1$.  Then $t(N) \geq t$.
\end{proposition}

Note that the quantities $\delta_2, \delta$ already made an appearance in \Cref{upper-crit}.  As we shall see, when $N$ is large we will be able to construct multisets $\tuple^{(0)}$ that make all the other quantities $\delta_i, i \neq 2$ and $\alpha_i$ quite small, which is how we shall establish such results as \Cref{main}(iii) and the lower bound in \Cref{main}(iv).
...
\begin{itemize}
\item Since the elements of $\tuple^{(0)}$ are at most $(t/K)^2$, all the elements removed in Step (a) are of the form $mp$ where $m \leq t/K$.
\item For each large prime $p$ considered in Step (b),  one has $\nu_p(N!) = \lfloor N/p \rfloor$ by \eqref{legendre} and \eqref{conditions}, while $\lceil t/p \rceil \leq K \leq t/K$ (again by \eqref{conditions}).  
\end{itemize}
...
$$ \nu_{p_1}\left(\frac{N!}{\prod \tuple^{(1)}}\right) = \nu_{p_1}\left(\frac{N!}{\prod \tuple^{(0)}}\right) + A_{p_1}.$$
...
$$ \nu_{p_1}\left(\frac{N!}{\prod \tuple^{(1)}}\right) = \nu_{p_1}\left(\frac{N!}{\prod \tuple^{(0)}}\right) + A_{p_1} - B_{p_1}.$$
...
\begin{itemize}
\item[(C)]  If an element in $\tuple^{(1)}$ is divisible by some small or medium prime $3 < p \leq t/K$, and one replaces $p$ by $\lceil p \rceil^{\langle 2,3 \rangle}_{4.5}$ in the factorization of that element, then the $p$-surplus decreases by one, while (by \Cref{power-lemma}) the $t$-excess increases by at most $\kappa_{4.5}$, and the quantity $\| (\nu_2(\prod \tuple^{(1)}),\nu_3(\prod \tuple^{(1)}))\|_\gamma$ increases by at most $\frac{2}{\log 12}(\log p + \kappa_{**})$.  All other $p_1$-surpluses or $p_1$-deficits for $p_1 \neq 2,3,p$ remain unaffected.
\item[(D)]  If one adds an element of the form $m \lceil t/m \rceil^{\langle 2,3 \rangle}_{4.5}$ to $\tuple^{(1)}$ for some $m \leq t/K$ that is the product of small primes $3 < p \leq K$, then the $p$-surpluses at small primes $p$ increase by $\nu_p(m)$, while (by \Cref{power-lemma}) the $t$-excess increases by at most $\kappa_{4.5}$, and the quantity $\sup_{p_0=2,3} \frac{p_0-1}{N} \nu_{p_0}\left(\frac{N!}{\prod \tuple^{(1)}}\right)$ increases by at most $\frac{2}{\log 12}(\log(t/m) + \kappa^*_{4.5})$.  The $p$-surpluses or $p$-deficits at medium or large primes remain unaffected.
\end{itemize}

With these observations in mind, we perform the following modifications to the multiset $\tuple^{(1)}$.
\begin{itemize}
\item[(c)] If there is a $p_1$-deficit $\nu_{p_1}(\prod \tuple^{(1)}/N!) > 0$ at some small or medium prime $3 < p_1 \leq t/K$, then we perform the replacement of $p_1$ in one of the elements of $\tuple^{(1)}$ with $\lceil p_1 \rceil^{\langle 2,3\rangle}_{4.5}$ as per observation (C), $\nu_{p_1}(\prod \tuple^{(1)}/N!)$ times, in order to eliminate all such deficits.
...
\end{itemize}
Call the multiset formed from $\tuple^{(1)}$ formed as the outcome of applying Steps (c), (d), (d') as $\tuple^{(2)}$.  The product of all the primes arising in (d') has logarithm equal to
...
=  \kappa_{4.5} \sum_{3 < p_1 \leq t/K} \left|\nu_{p_1}\left( \frac{N!}{\prod \tuple^{(0)}} \right) + A_{p_1} - B_{p_1} \right|_{0,1},$$
in Step (d) by at most
$$ \kappa_{4.5} \sum_{K < p_1 \leq t/K} \left|\nu_{p_1}\left( \frac{N!}{\prod \tuple^{(1)}} \right) \right|_{1,0}
=  \kappa_{4.5} \sum_{K < p_1 \leq t/K} \left|\nu_{p_1}\left( \frac{N!}{\prod \tuple^{(0)}} \right) + A_{p_1} \right|_{1,0},$$
and in Step (c) by at most
$$ \kappa_{4.5} (1 + \sum_{3 < p_1 \leq K} \left|\nu_{p_1}\left( \frac{N!}{\prod \tuple^{(0)}} \right) \right|_{\frac{\log p_1}{\log(t/K^2)},0}).$$
...
$$\frac{2}{N\log 12} \sum_{3 < p_1 \leq t/K} \left|\nu_{p_1}\left( \frac{N!}{\prod \tuple^{(0)}} \right) + A_{p_1} - B_{p_1} \right|_{0,\log p_1 + \kappa_{**}},$$
in Step (d) by at most
$$\frac{2}{N\log 12} \sum_{K < p_1 \leq t/K} \left|\nu_{p_1}\left( \frac{N!}{\prod \tuple^{(0)}} \right) + A_{p_1} \right|_{\log(t/p_1) + \kappa_{**},0},$$
and in Step (d') by at most the sum of
$$\frac{2}{N\log 12} \sum_{3 < p_1 \leq K} \left|\nu_{p_1}\left( \frac{N!}{\prod \tuple^{(0)}} \right) + A_{p_1}-B_{p_1} \right|_{\log(K^2) + \kappa_{**},0}$$
and
...
\begin{align*}
n - 2\gamma_2 m &= \nu_2(N!) - 2\gamma_2 \nu_3(N!) - (\nu_2(\prod \tuple^{(2)}) - 2\gamma_2 \nu_3(\prod \tuple^{(2)})) \\
&\geq \nu_2(N!) - 2\gamma_2 \nu_3(N!) - N (1-\gamma_2) \sum_{i=1}^6 \alpha_i \\
&> N - \lceil
 \frac{\log N}{\log 2}\rceil - \gamma_2 N - N (1-\gamma_2) (1 - \alpha_7) \\
&= N (1-\gamma_2) \alpha_7 - \lceil \frac{\log N}{\log 2}\rceil \\
&\geq 0
\end{align*}
and similarly
\begin{align*}
  2m - \gamma_3 n &= 2\nu_3(N!) - \gamma_3 \nu_2(N!) - (2\nu_3(\prod \tuple^{(2)}) - \gamma_3 \nu_2(\prod \tuple^{(2)})) \\
&\geq 2\nu_3(N!) - \gamma_3 \nu_2(N!) - N (1-\gamma_3) \sum_{i=1}^6 \alpha_i \\
&> N - 2\lceil \frac{\log N}{\log 3}\rceil - \gamma_3 N - N (1-\gamma_3) (1 - \alpha_7) \\ 
&= N (1-\gamma_3) \alpha_7 - 2\lceil \frac{\log N}{\log 3}\rceil \\
&\geq 0.
\end{align*}
...
\begin{align*}
  ...
  &\leq \frac{1}{\log t} (\nu_2(N!) \log 2 + \nu_3(N!) \log 3) \\
  &\leq \frac{1}{\log t} \left(N \log 2 + \frac{N}{2} \log 3\right) \\
  &= \frac{N \log 12}{2\log t}.
\end{align*}
...
We have
\begin{align*}
  2m_2 &\leq \frac{2}{\log 3} \log \lceil t/2^{n_0} \rceil^{\langle 2,3 \rangle} \\
  &\leq \frac{2}{\log 3} (\log(2L) + \kappa_L) 
\end{align*}
and
\begin{align*}
  n_2 &\geq n_0 \\
  &\geq \frac{\log t - \log(2L)}{\log 2};
\end{align*}
similarly
...
\end{split}
\end{equation}
This lets us compute the quantity $\kappa_{**}$.
...
\end{equation}
In practice, these expressions can be adequately controlled by \Cref{osc-lemma}.
...
\end{equation}
and $0 < c_1 < 1$ is a small absolute constant.  With this choice of parameters, we see from \eqref{delta-def}, \eqref{stirling} that
$$ \delta = \frac{ec_0}{\log N} + \frac{e}{\log^{1+c_1} N} + O\left( \frac{1}{\log^2 N} \right).$$

Let $N$ be sufficiently large.  We use the construction of the previous section with the parameters
\begin{align}
...
\end{equation}
The conditions \eqref{condition}, \eqref{t-lower} are easily verified for $N$ large enough.
...
Several of the quantities $\delta_i, \alpha_i$ can now be immediately estimated using \eqref{alpha1-vanish}, \eqref{delta1-bound}, \Cref{delta3-alpha3-bound}, and the prime number theorem:
\begin{align*}
...
\end{align*}
and the claim $t(N) \geq t$ now follows from \Cref{step4-reduce}.
...
where
$$
\tilde E(x) \coloneqq 0.95 \sqrt{x} + \frac{\sqrt{x}}{8\pi} \log x(\log x - 3) 1_{x \geq 10^{19}} + \min( \eps_0, \eps_1(x), \eps_2(x)) x 1_{x \geq e^{45}}$$
and
\begin{align*}
...
\end{align*}
From using the $\eps_1$ term, it is clear that
$$ \tilde E(x) \ll x \exp(-c \sqrt{\log x})$$
for some absolute constant $c>0$; and by using the $\eps_0$ term and routine calculations one can show that 
$$ \tilde E(x) \leq E(x)$$
for all $x \geq 1423$.

Observe that $E$ is monotone non-decreasing. Thus by \Cref{integ-lemma}, to show \eqref{bsharp} will suffice to show that
$$ \sum_{p \leq x} \log p = x - \sqrt{x} + O_{\leq}(E(x))
  = \int_0^x \left(1-\frac{2}{\sqrt{t}}\right)\ dt + O_{\leq}(E(x))$$
for all $x \geq 1423$.

For $1423 \leq x \leq 10^{19}$, this claim follows from \cite[Theorem 2]{buthe-2}.  In the range
...
\begin{remark} Assuming the Riemann hypothesis, the final term in the definition of $\tilde E(x)$ may be deleted, since \cite[(7.3)]{buthe} then holds for all $x \geq 5000$.  
\end{remark}
