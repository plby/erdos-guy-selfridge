...
\begin{itemize}
  ...
  \item The method also verifies this lower bound for $N=41006$, while the linear programming upper bound \eqref{lp-upper} shows that $t(N) \geq N/3$ fails for all smaller $N$ except for $N=1,2,3,4,5,6,9$; see \Cref{fig-surplus}.
...
\begin{figure}
  ...
  \caption{Exact values of $t(N)/N$ for $80 \leq N \leq 10^4$, obtained via linear programming upper bounds and floor+residual lower bounds (using integer programming to resolve gaps between these bounds).  The upper bound from \Cref{upper-crit} is surprisingly sharp, as is the refined asymptotic $1/e - c_0/\log N - c_1/\log^2 N$, though the cruder asymptotics $1/e$ or $1/e - c_0/\log N$ are significantly poorer approximations.}
  \label{fig-long}
  \end{figure}
...
$7 \times 10^8$ & $\num{246059851}$ & $t(N)_- + 89$ & $t(N)_- + 1147$ & $t(N)_- - \num{282167}$ \\
      \hline
\end{tabular}
\caption{For sample values of $N \in [10^5, 7 \times 10^8]$, the (remarkably precise) lower and upper bounds $t(N)_- \leq t(N) \leq t(N)_+$ obtained by floor+residuals and linear programming respectively, the (slightly weaker) upper bound on $t(N)$ from \Cref{upper-crit}, and the conjectural approximation $\frac{N}{e} - \frac{c_0 N}{\log N} - \frac{c_1N}{\log^2 N}$ (rounded to the nearest integer). }\label{long-table}
\end{table}
...
but the convergence rate here is slow.  To accelerate the convergence, we write $\lceil\frac{k}{e} \rceil = \frac{k}{e} + \{1-\frac{k}{e}\}$ and use the more precise Taylor approximation
$$ \frac{e \left\{\frac{k}{e}\right\}}{k^2} - \frac{e^2 \left\{\frac{k}{e}\right\}^2}{2k^3} 
\leq \frac{1}{k} \log\left(\frac{e}{k} \left\lceil \frac{k}{e} \right\rceil\right) \leq \frac{e \left\{1-\frac{k}{e}\right\}}{k^2}.$$
Bounding $\{1-k/e\}$ by one, we have the tail bound
$$ 0 \leq \sum_{k=K+1}^\infty \frac{e^2 \left\{1-\frac{k}{e}\right\}^2}{2k^3} \leq \frac{e^2}{4K^2}$$
so the main task is then to control the simplified tail
$$ \sum_{k=K+1}^\infty \frac{e\left\{1-\frac{k}{e}\right\}}{k^2}.$$
...
$$ \sum_{k=K+1}^\infty e\frac{\left\{1-\frac{k}{e}\right\}-\frac{1}{2}}{k^2}.$$
...
$$ \left| \sum_{k=K+1}^\infty e\frac{\left\{1-\frac{k}{e}\right\}-\frac{1}{2}}{k^2}\right| \leq \frac{1}{N+1} \sum_{k=K+1}^\infty \frac{e}{k^2}
