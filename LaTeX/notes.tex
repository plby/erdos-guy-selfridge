...
\end{equation}
for an explicit constant $c_1=0.7555\dots$; see \Cref{upper-bound}.
\end{itemize}
\end{theorem}
...
TT is supported by NSF grant DMS-2347850.  We thank Thomas Bloom for the web site \url{https://www.erdosproblems.com}, where the author learned of this problem, as well as Bryna Kra and Ivan Pan for  corrections.  We are particularly indebted to Andrew Sutherland for proposing and then implementing the greedy algorithm for this problem.
...
Throughout this paper, the symbol $p$ (or $p_0$, $p_1$, etc.) is always understood to be restricted to be prime.  The primes $2,3$ will play a special role\footnote{One could also run analogous arguments with other sets of tiny primes; for instance, the initial version \cite{tao} of this paper only utilized the prime $2$ in this fashion.} in this paper and will be referred to as \emph{tiny primes}. 
...
\begin{example} Suppose one wishes to factorize $5! = 2^3 \times 3 \times 5$.  The attempted $3$-admissible factorization $\tuple \coloneqq \{3,4,5,5\}$ has a $2$-surplus of $\nu_2(5!/\prod \tuple) = 1$, is in balance at $3$, and has a $5$-deficit of $\nu_2(\prod \tuple/5!) = 1$, so it is not a factorization or subfactorization of $5!$.  The $3$-excess of this multiset is
  ...
  and the accounting identity \eqref{accounting} becomes
...
Note that the quantities $\delta_2, \delta$ essentially appeared previously in \Cref{upper-crit}.  Informally, $\delta$ and the condition \eqref{delta-cond} represent the ``budget'' of what one can ``spend'' on various imperfections of the approximate factorization $\tuple^{(0)}$ until we become unable to ``afford'' to ``buy'' a $t$-admissible subfactorization of cardinality at least $N$.  The quantity $\delta_2$ represents the ``fixed expense'' coming from large primes that is unavoidable; but the other expenditures $\delta_i, i \neq 2$ can be made small by suitable exploitation of the pool of tiny primes present in the factorization of $N!$ (which multiply out to roughly $\sqrt{12}^N$).  The secondary constraint \eqref{alpha-cond} can roughly speaking be viewed as a requirement that one does not completely exhaust this pool.
...
$$
and hence by \eqref{kappastar-def},  \eqref{kappastar-2-def}, \eqref{kappastar-3-def} we have for sufficienty large $N$ that
$$ \kappa_{**} \ll 1.$$
...
$$ B_{p_1} \leq \sum_{m \leq K} \nu_{p_1}(m) \sum_{k: a_{k,m} < b_{k,m}} \frac{k}{\log(N a_{k,m})} \left( a_{k,m}-b_{k,m} + 2\frac{E(N_0 b_{k,m})}{N_0} \right)$$
and lower bound it by
$$ B_{p_1} \geq \sum_{m \leq K} \nu_{p_1}(m) \sum_{k: a_{k,m} < b_{k,m}} \frac{k}{\log(N b_{k,m})} \left( \left(1-\frac{2}{\sqrt{a_{k,m} N}}\right) (a_{k,m}-b_{k,m}) - 2\frac{E(N_0 b_{k,m})}{N_0 b_{k,m}} \right).$$
...
$$ \delta_7 \leq \frac{\kappa_{4.5} \log \sqrt{12}}{\log(10^{70}/3)}  \leq 0.02265 \delta$$
...
\begin{align*}
  ...
  &= 0.1797439053\dots;
\end{align*}
the convergence of this series can be accelerated by any number of methods, for instance by subtracting the asymptotically similar sum $\frac{1}{2e} \sum_{k=1}^\infty \frac{1}{k^2} = \frac{1}{2e} \frac{\pi^2}{6}$.  One also has
