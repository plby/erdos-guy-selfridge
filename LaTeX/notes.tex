...
\begin{figure}
  ...
  \caption{Lower bounds on $t(N)/N$ coming from the linear programming and greedy methods for selected $N$ in the range $10^4 \leq N \leq 10^{12}$, as well as upper bounds coming from linear programming and \Cref{upper-crit}.  Even if one simply takes floors from the linear program and discards residuals, the resulting lower bound is asymptotically almost indistinguishable from the upper bound. {\bf would be nice to extend the LP bounds back to cover the $10^4 \leq N \leq 10^5$}}\label{fig-verylarge}
\end{figure}
