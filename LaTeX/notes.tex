...
\begin{itemize}
  ...
  \item In \Cref{rearrange-sec}, we extend the \emph{rearrangement approach} from \cite{guy-selfridge} to give a computer-assisted proof that \Cref{main}(iii) holds for sufficiently large $N$.  We also give a computer-assisted proof of \Cref{main}(v) for $N\ge 2\times 10^6$.  Finally, we give an analytic proof of \eqref{t1}.
...
\Cref{main}(v) & $\num{26244} \leq N \leq 5 \times 10^6$ & Linear (or dynamic) programming \\
\hline
...
\Cref{main}(v) & $N \ge 2\times 10^6$ & Rearrangement \\
\hline
...
$$ a'_\ell \coloneqq \left\lfloor (a_\ell + \frac{1_{\ell \in S}}{\sqrt{\ell}}) N \right\rfloor / N,$$
...
\begin{proposition}\label{onefourth}  Let $N \ge 2\times 10^6$.  Then $t_{2,3}(N) < N/4$.
\end{proposition}
...
and
$$ C = \frac{1559}{24}$$
and one then we obtain the desired contradiction for $N \geq 2\cdot 10^6 > 1328148 = \lceil\frac{C}{\eps}\rceil$.

{\bf Link to code.  Mention verification for small N.}
\end{proof}
