...
We can now prove the upper bound portion of \Cref{main}(iv):
  ...
  \begin{proposition}\label{upper-bound}  For large $N$, one has
    ...
    On the interval $[1/\log N,1]$, the piecewise smooth function $f_{N/t}$ is bounded by $O(1)$ thanks to \eqref{falpha-bound}, and has a total variation of $O(\log N)$; the same is then true for the rescaled function $x \mapsto f_{N/t}(x/N)$ on $[N/\log N,1]$.  By \Cref{osc-lemma} (with classical error term), the left-hand side is then
...
\delta_7 &\coloneqq \frac{\kappa_* \log 12}{2\log t} \label{delta7-def}  \\
