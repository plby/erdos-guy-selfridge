...
The optimal dual weights $w^\star_p$ of the dual linear program~\eqref{pj}, \eqref{hyp} over all admissible subfactorizations $j \geq t$ of $N!$ with $t \leq N/2$ are weakly increasing.
\end{lemma}

\begin{proof}
It is well known that if an optimal solution to a linear program exists, there exists a \emph{basic} solution where up to $\pi(N)$ constraints hold with equality (some $w^\star_p$ could be fixed at their lower bound). Let $\mathcal{B}$ denote the set of basic factors $j$ whose associated constraint $\sum_p \nu_p(j) \cdot w^\star_p = 1$.
%From the discussion above on the domain $J_{t,N}$ it follows that all $j \in \mathcal{B}$ are bounded by $j \leq \max(N, t^2)$.

Assume that the weights $w^\star$ are weakly increasing up to prime $p$ and that $w^\star_p > w^\star_q$ for the next larger prime $q$. Since $w^\star_p > 0$, the basis $\mathcal{B}$ contains a factor $j$ with $\nu_p(j) > 0$. If factor $j \cdot q / p $ is admissible, its associated constraint has $\sum_i \nu_i(j \tfrac{q}{p}) \cdot w^\star_i < 1$, contradicting the assumption that $w^\star$ is optimal.

It remains to show that $j \cdot q / p $ is admissible. If $\nu_q(j) < \nu_q(N!)$ it clearly is. Thus, assume that $\nu_q(j) = \nu_q(N!)$. It holds that $q^{\nu_q(N!)} > N/2 \geq t$ for all primes $3 \leq q \leq N$ showing that $j$ could not be a basic factor in this case.
\end{proof}
