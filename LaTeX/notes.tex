...
\begin{proposition}\label{step4-reduce}  Let $L \geq 1$.  Let $9L < t = N/e^{1+\delta}$ for some $\delta>0$ be such that \eqref{condition} holds, and suppose that the algorithm reaches the end of Step 3 to produce a multiset $\tuple^{(3)}$ such that the quantities
  ...
  \end{align}
    obeying the following hypotheses involving:
  \begin{itemize}
...
&\quad + (\log K^2 +\kappa_K) Y_1^+ + (\log K + \kappa_5) Y_1^- +  (\log(t/K) +\kappa_K) (Y_2^\pm+1)  
\\
...
\frac{1}{N} \excess_t(\tuple^{(3)}) \leq 
 \frac{1}{N}\excess_t(\tuple^{(2)}) + \int_{t/NK}^1 f_{N/t}(x)\ dx + \|f_{N/t}\|_{\mathrm{TV}^*((t/NK,1])} \frac{E(N)}{N}.  
\end{equation}
...
\begin{equation}\label{step2-small}
  \nu_{p_1}\left(\frac{N!}{\prod \tuple^{(2)}}\right) = \nu_{p_1}\left(\frac{N!}{\prod \tuple^{(1)}}\right) +
  A \sum_{m \leq K}^* \nu_{p_1}(m) \left(\pi\left(\frac{t(1+\sigma)}{m}\right) - \pi\left(\frac{t}{m}\right)\right) + A N Z_{p_1}
\end{equation}
where
$$ Z_{p_1} \coloneqq 
\frac{1}{N} \sum_{K < m \leq K(1+\sigma)}^* \nu_{p_1}(m) \left(\pi\left(\frac{t(1+\sigma)}{m}\right) - \pi\left(\frac{t}{K}\right)\right).$$
...
&\leq 
\frac{3N}{2tA} + \frac{4}{N} + \int_{t/NK}^1 f_{N/t}(x)\ dx + \|f_{N/t}\|_{\mathrm{TV}^*((t/NK,1])} \frac{E(N)}{N} \label{excess-3-bound}\\
Y_1^+ &\leq \frac{4A+3}{3N} \sum_{3 < p_1 \leq K} 
 \left\lceil \frac{\log N}{\log p_1} \right\rceil \frac{\log p_1}{\log (t/K^2)} + A \sum_{3 < p_1 \leq K} Z_{p_1} \frac{\log p}{\log(t/K^2)} \\
 &\quad + \sum_{3 < p \leq K}  |W_{p_1}|_{\frac{\log p}{\log (t/K^2)}, 0} \label{y1p-bound}\\
Y_1^- &\leq \frac{4A+3}{3N} \sum_{3 < p_1 \leq K} 
\left\lceil \frac{\log N}{\log p_1} \right\rceil 
+ \sum_{3 < p \leq K}  |W_{p_1}|_{0, 1} \label{y2p-bound} \\
Y^\pm_2 &\leq \frac{4A+3}{3N} \sum_{K < p \leq t/K} \left\lceil \frac{\log N}{\log p_1} \right\rceil \\
...
A key point will be that in practice, the $W_{p_1}$ will be positive (or only barely negative), so that their contribution to \eqref{y2p-bound} vanishes (or is small).
...
\begin{equation}\label{nupsum}
  ...
  \end{split}
\end{equation}
we thus see that the first term on the left-hand side of \eqref{second-criterion'} is acceptable.  It will now suffice to show that
\begin{equation}\label{xb} X'_1, X'_2 \ll \frac{N (\log\log N)^3}{\log^2 N}.
\end{equation}
From \eqref{x11-est} and the prime number theorem we have
$$ \frac{4A+3}{3} \sum_{3 < p \leq t/K}  \left\lceil \frac{\log N}{\log p} \right\rceil \ll A \frac{t/K}{\log N} \ll \frac{N}{\log^2 N}$$
...
$$ \frac{4A+3}{3} \sum_{3 < p \leq K} \left\lceil \frac{\log N}{\log p} \right\rceil \ll A (\log N) \frac{K}{\log N} \ll \log^6 N
$$
